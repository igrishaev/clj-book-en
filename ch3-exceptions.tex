...
For Clojure, \footurl{Sentry-clj}{https://github.com/getsentry/sentry-clj}[Sentry-clj][-22mm] and \footurl{Exoscale Raven}{https://github.com/exoscale/raven}[Exoscale Raven] libraries are available. The former relies on the official Java library. Add it to your project:
...
In response, we will receive the event number, and a new entry will appear in the project. Suppose we passed \emph{e}, which is a chain of three exceptions defined above \page{e-sample}. In the Sentry interface, we will see information about each level. The \verb|:extra| field will contain the data of the top-level exception~--- the map \verb|{:user-id 5}|.
...
Open the event in Sentry and scroll down. In the \verb|extra| section, the \verb|:via| key with detailed information will appear. Below is a snippet of it \lis{fig:ex-json-data}. Each map consists of an exception class, message, and data. The \verb|:at| field is a vector of four elements: a class, a method, a filename, and the line where an exception was thrown. Such a report makes it easier to find the cause.
...
The \verb|if/else| cascade works, but looks unwieldy. The issue is also known as the pyramid of doom. The deeper the nesting, the more problems the developer has. For fun, add another condition to the middle of the Clojure code \lis{fig:clj-pyramid}:
...
To make the scheme work, let's add the \verb|wrap-http-response| decorator. It catches errors from functions with an exclamation mark, extracts an answer from them, and returns it to a client. Thus, calling \code{(not-found!)} in a handler will lead to the 404 Not Found response but not 500 which denotes a server failure.
...
It is important to remember that the code from \code{finally} will not be included in the result. The \code{try} expression is designed so that if an error occurs, the result comes from one of the \code{catch} branches. The \code{finally} branch serves only for side effects. This is best seen in the example with printing:
...
This order means that Clojure will first execute the code from \code{catch} (print ``catch'') and remember the result (keyword \code{:result-catch}). Then it will execute \code{finally} (print <<finally>>) and discard the result (keyword \code{:result-finally}). The \code{:result-catch} keyword will be the result of the whole \code{try}.

For comparison, let's give an unfortunate example from practice. It was necessary to perform some action and, regardless of the outcome, return a positive result. The programmer wrapped the code into the \code{try/catch} block and logged the exception. However, he put the result ino the \code{finally} branch, which caused strange behavior: \code{nil} was returned instead of a map.
...
Apparently, the programmer decided that \code{finally} is literally the final result of \code{try}. This is not entirely true: \code{finally} will actually work last, but \code{try} will return either the result of the main code or one of the \code{catch} branches.
...
The \verb|try/catch| system relies on classes and inheritance. If you need to catch a particular exception, write a class and throw an instance of it. A typical Java or Python project carries a module with custom exceptions. Usually, there is a \code{ProjectException} class, and others like \code{UserNotFound} or \code{AccessDenied} inherited from it.

\index{libraries!Slingshot}

In the chapter on Spec, we've said that predicates were more powerful than types \page{type-and-pred}. The same is true for exceptions: if you can catch them with a function, you don't need a class. The \footurl{Slingshot}{https://github.com/scgilardi/slingshot}[Slingshot][-6mm] library offers a predicate approach. There are improved versions of \verb|try|, \verb|catch| and \verb|throw| macros in Slingshot.
...
When the map is thrown, usually we add the \verb|:type| field to the map \coderef{2}. It specifies a key with the current namespace, in our case~--- \verb|::user-error|. When catching exceptions in another module, the selector looks like this: \spverb|[:type :book.exceptions/user-error]|. The namespace guarantees that we will not intercept \texttt{:user\--error} of someone else's library.
...
Unfortunately, there is neither the \verb|ChecksumError| class nor an equivalent in SDK. Instead, we'll receive a regular \verb|AmazonS3Exception| with a long text. To highlight our case, let's compare the message with the template. Let's make a check function first:
...
Slingshot is not the only example of exceptions for Clojure. Pay attention to \footurl{Ex}{https://github.com/exoscale/ex}[Ex][-3mm]~--- the analog developed at Exoscale. The library takes into account the semantics of the keys (a failure, conflict, etc), and their inheritance, supports Manifold, and much more.
...
A different semantics is popular in JavaScript: callback functions take the \verb|error| and \verb|result| arguments. The \verb|pcall-js| function is a modified version of \verb|pcall| that returns an \tuple {error, result} pair. When the first item if the pair is not \code{nil}, it means the call failed.
